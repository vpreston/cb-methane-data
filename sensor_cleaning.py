...
def handle_nitrate_time(data, day, month, year):
    '''
    Handle seperating the timestamp object into individual data columns
    @input: dataframe
    @output: none; dataframe rewritten in processing
    '''
    data.loc[:,'Year'] = data.apply(lambda x : year,axis=1)
    data.loc[:,'Month'] = data.apply(lambda x : month,axis=1)#int(str(x['2018088'])[4:])/30+1,axis=1)
    data.loc[:,'Day'] = data.apply(lambda x : day,axis=1)#int(str(x['2018088'])[4:])%30-4,axis=1)
    data.loc[:,'Hour'] = data.apply(lambda x : int(x['time']),axis=1)
    data.loc[:,'Minute'] = data.apply(lambda x : int((float(x['time']) - float(x['Hour']))  * 60),axis=1)
    data.loc[:,'Second'] = data.apply(lambda x : float(((float(x['time']) - float(x['Hour'])) * 60 - float(x['Minute'])) * 60),axis=1)
...
def clean_gga(data, offset_time=0.0014, min_time=None, max_time=None):
    '''
    ...
    data.loc[:,'Julian_Date_cor'] = data.apply(lambda x: x['Julian_Date']+offset_time,axis=1)
...
def clean_optode(data, offset=2440678.484, min_time=None, max_time=None):
    '''
    ...
    data.loc[:,'Julian_Date'] = data.apply(lambda x : x.posixtime / 86400.0 + offset - 0.0416600001, axis=1)
...
def clean_nitrate(data, day, month, year, min_time=None, max_time=None):
    '''
    ...
    handle_nitrate_time(data, day, month, year)
    data = data.drop(data[data['data'] <= 0].index)
